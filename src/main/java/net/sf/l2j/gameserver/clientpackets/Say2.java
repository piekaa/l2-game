...
public final class Say2 extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (_type == 0 && _text.startsWith(".")) {
            ...
            if (_text.equals(".drop")) {
                var target = getClient().getActiveChar().getTarget();

                if (target != null && target instanceof L2NpcInstance) {
                    var mob = (L2NpcInstance) target;
                    var dropList = mob.getTemplate().getDropList();
                    var spoilList = mob.getTemplate().getSpoilList();

                    StringBuilder responseMessage = new StringBuilder("<br>Drop List:<br>");

                    for (String dropEntry : dropList) {
                        responseMessage.append(dropEntry).append("<br>");
                    }

                    responseMessage.append("<br>Spoil List:<br>");

                    for (String spoilEntry : spoilList) {
                        responseMessage.append(spoilEntry).append("<br>");
                    }

                    NpcHtmlMessage htmlMessage = new NpcHtmlMessage(target.getObjectId());
                    htmlMessage.setHtml("<html><body>" + responseMessage + "</body></html>");

//                    CreatureSay message = new CreatureSay(target.getObjectId(), ALL, target.getName(), responseMessage.toString());
                    getClient().sendPacket(htmlMessage);
                }
            }
        }
