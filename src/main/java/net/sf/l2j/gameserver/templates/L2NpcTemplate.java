...
public final class L2NpcTemplate extends L2CharTemplate {
    ...
    public List<String> getDropList() {
        ...
        for (L2DropCategory category : _categories) {
            ...
            dropList.addAll(toDropList(category));
        }
        return dropList;
    }

    public List<String> getSpoilList() {
        List<String> dropList = new ArrayList<>();
        for (L2DropCategory category : _categories) {
            if (!category.isSweep()) {
                continue;
            }
            dropList.addAll(toSpoilList(category));
        }
        return dropList;
    }

    private List<String> toDropList(L2DropCategory category) {
        return category.getAllDrops().stream().map(drop -> {
            float rate = drop.isAdena() ? Config.RATE_DROP_ADENA : Config.RATE_DROP_ITEMS;
            float chance = drop.getChance() * rate / L2DropData.MAX_CHANCE * 100;

            return dropOrSpoilListEntry(chance, drop);
        }).collect(Collectors.toList());
    }

    private List<String> toSpoilList(L2DropCategory category) {
        System.out.println(Config.RATE_DROP_SPOIL);
        return category.getAllDrops().stream().map(drop -> {
            float chance = drop.getChance() * Config.RATE_DROP_SPOIL / L2DropData.MAX_CHANCE * 100;
            return dropOrSpoilListEntry(chance, drop);
        }).collect(Collectors.toList());
    }

    private String dropOrSpoilListEntry(float chance, L2DropData drop) {
        var smallFormatter = new DecimalFormat("0.0000000");
        var normalFormatter = new DecimalFormat("0.00");
        var chanceText = "";

        if (chance < 1) {
            chanceText = smallFormatter.format(chance);
        } else {
            chanceText = normalFormatter.format(chance);
        }

        var name =ItemTable.getInstance().getTemplate(drop.getItemId()).getName();
        var range = "" + drop.getMinDrop();
        if (drop.getMinDrop() != drop.getMaxDrop()) {
            range =  drop.getMinDrop() + "-" + drop.getMaxDrop();
        }

        return range + " " + name + " " + chanceText + "%";
    }
